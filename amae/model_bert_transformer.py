...
class ESIM(nn.Module):
    ...
    def __init__(self,
        ...
        self._composition = Transformer(d_model=self.hidden_size, dim_feedforward=self.hidden_size*4)
    ...
    def forward(self, premises, hypotheses):
        ...
        encoded_hypotheses = self._encoding(hypotheses, hypotheses_lengths)

        projected_premises, projected_hypotheses = self._attention(encoded_premises, premises_mask,
                                                                   encoded_hypotheses, hypotheses_mask)

        projected_premises = projected_premises.transpose(1, 0)
        projected_hypotheses = projected_hypotheses.transpose(1, 0)
        v_ai = self._composition(projected_premises, projected_premises).transpose(1, 0)
                                 # src_key_padding_mask=(1-premises_mask).byte(),
                                 # tgt_key_padding_mask=(1-premises_mask).byte()).transpose(1, 0)
        v_bj = self._composition(projected_hypotheses, projected_hypotheses).transpose(1, 0)
                                 # src_key_padding_mask=(1-hypotheses_mask).byte(),
                                 # tgt_key_padding_mask=(1-hypotheses_mask).byte()).transpose(1, 0)
