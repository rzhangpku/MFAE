...
from mfae.model_elmo import ESIM
...
def main(train_file,
    ...
    with open(os.path.normpath(os.path.join(script_dir, valid_files["matched"])), "rb") as pkl:
        ...
        matched_valid_loader = DataLoader(ElmoDataset(valid_data), batch_size=1, shuffle=False)

    with open(os.path.normpath(os.path.join(script_dir, valid_files["mismatched"])), "rb") as pkl:
        valid_data = pickle.load(pkl)
        valid_data = transform_elmo_data(valid_data, batch_size=batch_size, shuffle=False)
        mismatched_valid_loader = DataLoader(ElmoDataset(valid_data), batch_size=1, shuffle=False)
...
if __name__ == "__main__":
    default_config = "../../config/training/mnli_training_elmo.json"
    ...
    parser.add_argument("--checkpoint",
                        default=os.path.dirname(os.path.realpath(__file__)) + '/data/checkpoints/MNLI/elmo/' +"esim_{}.pth.tar".format(4),
